...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;사용한 데이터셋에서 정상 데이터(875,382개)와 고장 데이터(761개)의 개수 차이가 크기 때문에 전체 데이터의 정확도만으로는 모델의 성능을 정확하게 평가하기 어렵다. 전체 데이터를 정확도만으로 판단한다면 모든 데이터를 정상으로 예측한 경우 약99.91%의 정확도를 가지게 된다. 그렇기 때문에 4가지의 경우를 나누어서 모델의 성능을 평가하였다. 아래 사진을 통하여 약 95.5%의 정확도를 가진 모델임을 알 수 있다.
